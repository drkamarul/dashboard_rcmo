...
# Convert year from str to dbl in penerbitan_data
penerbitan_data <- penerbitan_data |> mutate(YEAR = as.numeric(YEAR))
...
kepakaran_data <- 
  kepakaran_data |> 
  select(staff_id, scopus_citations, scopus_publications, scopus_hindex)
```
...
```{r}
staff_demography_data_2024 <- staff_demography_data |> 
  select(staff_id, year, name, grade, cohort, academic_qualification_phd_master, 
  authors_id_scopus, department_faculty, s_t_non_s_t,
  status_1_active_study_leaves_sabbatical_training_attachment_seconded,
  status_2_permanent_contract, status_3_full_time_part_time ,status_4_active_staff_yes_no,
  citizenship_local_foreign, country) |> 
    filter(year == 2024)
```
...
penerbitan_data <- 
  penerbitan_data |> 
  select(staff_id, year, type_of_publication,  q1_or_q2_only_blank_if_no_data, database_source_scopus_wos_era)
```
...
count_publications_2024 <- penerbitan_data |>
  ...
  pivot_wider(
    names_from = type_of_publication,
    values_from = publications_count,
    values_fill = 0
  ) |>
  arrange(staff_id)
```
...
count_grants_2024 <- geran_data |>
  filter(year == 2024) |>
  group_by(staff_id, year, category) |>
...
## Merge count datasets for 2024

```{r}
merged_counts_2024 <- count_publications_2024 |> 
  full_join(count_grants_2024, by = "staff_id") |> 
  full_join(count_innovations_2024, by = "staff_id") |> 
  select(!(year)) |> 
  rename(year = year.x) |>
...
```{r}
merged_count_2024_demography <- merged_counts_2024 |>
  full_join(staff_demography_data_2024, by = "staff_id") |>
   filter(!is.na(year.x)) |>
    rename(year = year.x)
```